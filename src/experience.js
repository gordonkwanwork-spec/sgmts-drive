...
import {LENGTH,STOPS,sample,fromChainage,toChainage,LOOP,roadSection,project,projectFrame,END_STOP,CROSSINGS,cycleOffset,cycleHeight,DEPOT,laneOffset,curveRadius,returnOffset,CROSSOVER_START,cycleBridgeHeight,CYCLE_BRIDGES,cycleSample,footpathHeight} from './alignment.js';
import {buildEnvironment,JUNCTIONS,UNDERPASSES,lampFade,sceneryClear,terrainLevel,renderedGround} from './environment.js';
import {buildLots,occupiedBy,nightMaterials as lotNightMaterials} from './lots/massing.js';
import {PARKED_BAYS,DEPOT_Y} from './depot.js';
...
function streetModel(name){const model=streetKit.getObjectByName(name);if(!model)throw Error("Missing Blender model "+name);const group=model.clone(true);group.traverse(n=>{if(n.isMesh){n.castShadow=true;n.receiveShadow=true;}});
 // People carry their own materials so each can take baked lamp glow at night (the 6 real lights cannot reach everyone).
 if(/^(pedestrian_|cyclist|scooter)/.test(name)){group.traverse(n=>{if(n.isMesh&&n.material){n.material=n.material.clone();n.material.emissive?.set(0);}});litPeople.push(group);}
 return group;}
const litPeople=[];let peopleGlowTick=0,peopleWereLit=false;
// Baked-style lamp glow on people: the brightest nearby lamp within ~14 m of a person lifts their materials at night.
function glowPeople(night){
 if(!night){if(peopleWereLit){for(const g of litPeople)g.traverse(n=>{if(n.isMesh&&n.material.emissive)n.material.emissive.set(0);});peopleWereLit=false;}return;}
 if(peopleGlowTick++%15)return;peopleWereLit=true;
 const lamps=environment.lampPositions.filter(l=>Math.abs(l.x-camera.position.x)<180&&Math.abs(l.z-camera.position.z)<180);
 for(const g of litPeople){if(!g.visible||g.position.distanceToSquared(camera.position)>170*170)continue;const p=g.getWorldPosition(new T.Vector3());let glow=0;
  for(const l of lamps){const h=l.y-p.y;if(h<1.5||h>16)continue;const d2=(l.x-p.x)**2+(l.z-p.z)**2,r=l.kind==='mast'?24:13;if(d2<r*r*4)glow=Math.max(glow,Math.exp(-d2/(r*r)));}
  g.traverse(n=>{if(n.isMesh&&n.material.emissive)n.material.emissive.copy(n.material.color).multiplyScalar(glow*.55);});}}
...
// Northbound services show 產業園 at both ends, southbound 泥圍 (LED orange, unlit by scene lighting).
const DEST_GEOMETRY=new T.PlaneGeometry(.62,.2).rotateX(-Math.PI/2),DEST_MATERIALS=Object.fromEntries(['產業園','泥圍'].map(text=>{const c=document.createElement('canvas');c.width=512;c.height=160;const x=c.getContext('2d');
 x.fillStyle='#120a04';x.fillRect(0,0,512,160);x.font="bold 116px 'PingFang HK','Noto Sans CJK TC','Microsoft JhengHei',sans-serif";x.textAlign='center';x.textBaseline='middle';x.shadowColor='#ff8a1c';x.shadowBlur=18;x.fillStyle='#ffb347';x.fillText(text,256,84);
 const t=new T.CanvasTexture(c);t.colorSpace=T.SRGBColorSpace;t.anisotropy=4;return [text,new T.MeshBasicMaterial({map:t,toneMapped:false})];}));
function showDestination(tram,northbound){const text=northbound?'產業園':'泥圍';if(tram.destText===text)return;tram.destText=text;for(const m of tram.destSigns||[])m.material=DEST_MATERIALS[text];}
function setupTram(gltf,isAI=false){
 ...
 // Destination LEDs: the model's fixed text meshes (front 泥圍, rear 產業園) become one switchable sign per end.
 const destSigns=[];for(const g of sections)g.traverse(n=>{if(n.isMesh&&/^Route_destination/.test(n.name)){n.geometry=DEST_GEOMETRY;n.material=DEST_MATERIALS['泥圍'];destSigns.push(n);}});
 return {sections,doors,wheels,lights,cabDisplays,steering,glazing,wipers,destSigns,yaws:[0,0,0],isAI,s:1000,lat:-1.9,v:7,door:0};
}
...
// E-scooter riders on the footpaths: a standing pedestrian on a procedural stand-up scooter (deck, stem, bars, wheels).
const scooters=[];
function scooterGroup(i){const g=new T.Group();g.name='scooter-rider';const dark=new T.MeshStandardMaterial({color:[0x22262a,0xe7e9ea,0x2f6fb3,0xd9452f][i%4],roughness:.5,metalness:.3}),tyre=new T.MeshStandardMaterial({color:0x151515,roughness:.9});
 const part=(geo,mat,x,y,z,rx=0)=>{const m=new T.Mesh(geo,mat);m.position.set(x,y,z);m.rotation.x=rx;m.castShadow=true;g.add(m);return m;};
 part(new T.BoxGeometry(.17,.05,.62),dark,0,.14,0);part(new T.CylinderGeometry(.018,.018,1.02),dark,0,.63,-.33,-.12);part(new T.CylinderGeometry(.015,.015,.48),dark,0,1.12,-.39,0).rotation.z=Math.PI/2;
 const wheels=[-.36,.3].map(z=>part(new T.CylinderGeometry(.1,.1,.05,14).rotateZ(Math.PI/2),tyre,0,.1,z));
 const rider=streetModel('pedestrian_'+(i*3+1)%PEDESTRIAN_COUNT);rider.position.set(0,.17,.03);rider.userData.scooterRider=true;g.add(rider);g.userData.rider=rider;g.userData.wheels=wheels;return g;}
function makeScooters(){for(let i=0;i<14;i++){const g=scooterGroup(i);scene.add(g);scooters.push({group:g,s:(i+.5)/14*LENGTH,side:i%2?1:-1,dir:i%3?1:-1,v:3.6+(i%4)*.5,lean:0});}}
function scooterOk(s){const r=sample(s);return !r.elevated&&s>40&&s<END_STOP-40&&!STOPS.some(st=>Math.abs(st.s-s)<st.footprintLength/2+48)&&!JUNCTIONS.some(j=>Math.abs(j.s-s)<j.halfWidth+14)&&Math.abs(s-DEPOT.gate)>20;}
function updateScooters(dt){const subject=viewS();for(const c of scooters){let next=c.s+c.dir*c.v*dt;if(!scooterOk(next)){c.dir*=-1;next=c.s;if(!scooterOk(next)){c.s=(c.s+97)%LENGTH;continue;}}c.s=next;
  const g=c.group;g.visible=Math.abs(c.s-subject)<500;if(!g.visible)continue;const r=sample(c.s),lat=c.side*(roadSection(c.s).right+1.2+(c.dir===c.side?0:.9));
  g.position.set(r.x+r.lx*lat,r.y+footpathHeight(c.s),r.z+r.lz*lat);g.rotation.y=r.heading+(c.dir===1?0:Math.PI);for(const w of g.userData.wheels)w.rotation.x-=c.v*dt/.1;posePedestrian(g.userData.rider,0,dt);}}
...
function lightStreetObjects(night){
 ...
 // The few real lights go to lamps nearest the people in view, not to the camera: people 30–60 m ahead at a platform
 // or crossing were left unlit when every light sat on lamps around the camera (pavement glow alone is baked).
 const people=[...walkerObjects,...crossers,...plazaPeople,...cyclists].map(a=>a.group).filter(g=>g&&g.visible&&g.position.distanceToSquared(camera.position)<130*130).map(g=>g.position);
 const lamps=environment.lampPositions.map(p=>({p,d:camera.position.distanceTo(p)})).filter(q=>q.d<140).map(q=>{let near=Infinity;for(const g of people){const dx=g.x-q.p.x,dz=g.z-q.p.z;near=Math.min(near,dx*dx+dz*dz);}return {...q,score:people.length?Math.sqrt(near)+q.d*.08:q.d};});
 const fade=lampFade(lamps.map(q=>q.score),streetLights.length),lit=lamps.map((q,i)=>({...q,k:fade[i]})).filter(q=>q.k>0).sort((a,b)=>a.score-b.score);
...
function lightScene(){const night=state.condition==='night';glowPeople(night);if(night){lightRoadUsers(true);lightStreetObjects(true);}for(const l of headlights){l.intensity=night&&state.mode!=='free'?120:0;if(vehicle){l.position.copy(vehicle.sections[0].localToWorld(new T.Vector3(l.userData.side*.85,1,-5.4)));l.target.position.copy(vehicle.sections[0].localToWorld(new T.Vector3(l.userData.side*1.5,.1,-45)));}}scene.traverse(n=>{if(n.isLight&&n!==sun&&n!==ambient){if(!night)n.intensity=0;n.visible=night&&n.name!=='Tuen Ma headlight beam';}});}
...
function show(screen){if(screen==='menu'){resetAvatar();if(['driving','paused','complete'].includes(state.screen))menuStage('choose');}if(screen!=='driving')document.exitPointerLock?.();state.screen=screen;$('#menu').classList.toggle('hidden',screen!=='menu');$('#hud').classList.toggle('hidden',!['driving','paused'].includes(screen));$('#pause-sheet').classList.toggle('hidden',screen!=='paused');$('#report').classList.toggle('hidden',screen!=='complete');document.body.dataset.screen=screen;for(const el of document.querySelectorAll('#camera,#score-panel,#cluster,#route-strip,#buttons,#dwell'))el.classList.toggle('hidden',state.mode==='free');}
...
// One style for every shortcut: the corner badge below (no inline key chips).
for(const [sel,key] of [['#resume','Enter'],['#pause-sheet .menu-return','⌫'],['#report .menu-return','Enter'],['#explore-tail','E'],['#begin','Enter']])$(sel).setAttribute('aria-keyshortcuts',key);
...
// Every button shows its keyboard shortcut as a corner badge (CSS ::after on data-hotkey, so it survives label changes).
{const byAction={...buttonKeys,camera:'C',side:'X',indicator:'Z',pause:'P',sound:'M'},byPad={Enter:'Enter',KeyE:'E',Space:'Space',ControlLeft:'Ctrl'},byId={'close-options':'Esc','menu-music':'M'};
 for(const b of document.querySelectorAll('button')){const key=b.getAttribute('aria-keyshortcuts')||byAction[b.dataset.action]||byPad[b.dataset.key]||byId[b.id];if(!key)continue;
  b.dataset.hotkey=key;b.setAttribute('aria-keyshortcuts',key);if(!b.title.includes(' · '))b.title=(b.getAttribute('aria-label')||b.textContent.trim())+' · '+key;if(getComputedStyle(b).position==='static')b.style.position='relative';}}
...
// Opening: title (any key / tap) → chooser. Option buttons drive the hidden selects the rest of the game reads.
const menuStage=stage=>{$('#menu').dataset.stage=stage;document.body.dataset.menuStage=stage;};
function syncOptions(){for(const row of document.querySelectorAll('.option-row')){const sel=$('#'+row.dataset.for);for(const b of row.querySelectorAll('button'))b.setAttribute('aria-pressed',String(b.dataset.value===sel.value));}}
for(const row of document.querySelectorAll('.option-row'))for(const b of row.querySelectorAll('button'))b.onclick=()=>{const sel=$('#'+row.dataset.for);sel.value=b.dataset.value;sel.dispatchEvent(new Event('change'));syncOptions();};
$('#title-screen').onclick=()=>menuStage('choose');syncOptions();menuStage('title');
// Phones: every label stays on one line, shrinking its font (not below 8 px) until it fits its box.
const FIT_ONE_LINE='.emblem-top,.press,.option-row b,.option-row small,#menu-music,#begin,#hud-stack #toast,#hud-stack #instruction,#route-choices button,#next,#next-distance,#service-label,.cell span,.cell b,#mobile-buttons small';
function fitOneLine(only){if(!mobile)return;for(const e of only instanceof Set?only:document.querySelectorAll(FIT_ONE_LINE)){if(!e.getBoundingClientRect().width)continue;e.style.whiteSpace='nowrap';e.style.removeProperty('font-size');let size=parseFloat(getComputedStyle(e).fontSize);
 const room=()=>{const box=e.matches('#hud-stack *')?innerWidth-24:e.clientWidth;return e.scrollWidth>box+1;};while(room()&&size>8){size-=.5;e.style.setProperty('font-size',size+'px','important');}}}
if(mobile){addEventListener('resize',()=>requestAnimationFrame(()=>fitOneLine()));new MutationObserver(list=>{const hit=new Set();for(const m of list){const n=m.target.nodeType===3?m.target.parentElement:m.target;const e=n?.closest?.(FIT_ONE_LINE);if(e)hit.add(e);else if(m.type==='attributes'&&m.attributeName!=='class'||n?.id==='menu')hit.add(null);}
  if(hit.has(null))requestAnimationFrame(()=>fitOneLine());else if(hit.size)requestAnimationFrame(()=>fitOneLine(hit));}).observe(document.body,{subtree:true,childList:true,characterData:true,attributeFilter:['data-stage','hidden']});requestAnimationFrame(()=>fitOneLine());}
...
const viewS=()=>state.screen==='menu'&&attract.subject?attract.subject.s:state.avatar!=='drive'?player.s:state.mode==='free'?state.freeS:state.s;
...
// Menu attract mode: live AI service with a cinematic camera. Cuts every 7 s between five shots and hops to another
// moving tram every second cut: low lead (key-art hero angle), side dolly, rising crane, slow orbit, low chase.
const attract={subject:null,shot:-1,since:99,cut:false,frame:null,fov:40};
function attractShot(dt){
 const moving=ai.filter(a=>a.active&&a.v>2);attract.since+=dt;
 if(!attract.subject||!attract.subject.active||attract.since>7){if(!moving.length)return false;attract.shot=(attract.shot+1)%5;
  if(!attract.subject||!attract.subject.active||attract.shot%2===0){const others=moving.filter(a=>a!==attract.subject);attract.subject=(others.length?others:moving)[Math.floor(Math.random()*(others.length||moving.length))];}
  attract.since=0;attract.cut=true;}
 const a=attract.subject,f=a.sections[0].position,r=a.sections[a.sections.length-1].position,dir=f.clone().sub(r).setY(0).normalize(),left=new T.Vector3(dir.z,0,-dir.x),t=attract.since,y=f.y;
 const at=(d,l,h)=>f.clone().addScaledVector(dir,d).addScaledVector(left,l).setY(y+h);
 const shots=[
  ()=>({target:at(26-t*1.2,3.2,1.1),focus:at(0,-.4,2.4),fov:34}),       // low, head-on: headlights charging at the lens
  ()=>({target:at(10-t*3.2,10.5,4.6),focus:at(-2,0,1.8),fov:38}),           // side dolly past the articulations
  ()=>({target:at(-30,-9,4+t*3.4),focus:at(25,0,1.5),fov:46}),         // crane rising behind, looking up the line
  ()=>{const ang=.6+t*.22;return {target:f.clone().addScaledVector(dir,Math.cos(ang)*24).addScaledVector(left,Math.sin(ang)*24).setY(y+10),focus:at(-6,0,2),fov:40};},
  ()=>({target:at(-16,2.2,3.4),focus:at(40,0,2.6),fov:44})];           // low chase
 attract.frame=shots[attract.shot]();attract.fov=attract.frame.fov;return true;}
function aim(dt){const g=vehicle.sections[0],p=g.position,dir=new T.Vector3(0,0,vehicle.flipped?1:-1).applyQuaternion(g.quaternion),left=new T.Vector3(-1,0,0).applyQuaternion(g.quaternion);let target,focus;
 if(state.screen==='menu'&&attractShot(dt)){({target,focus}=attract.frame);camera.fov=attract.fov;camera.near=.15;camera.updateProjectionMatrix();}
 else if(state.screen==='menu'){target=p.clone().addScaledVector(dir,23).addScaledVector(left,-3.5);target.y+=3.6;focus=p.clone().addScaledVector(dir,-14).addScaledVector(left,7);focus.y+=1.3;}
 ...
 const k=firstCamera||attract.cut||state.mode==='free'||state.avatar!=='drive'||state.cam===1?1:1-Math.exp(-dt*(state.cam===1?18:4));attract.cut=false;if(firstCamera)resetView();camPos.lerp(target,k);look.lerp(focus,k);
 ...
 const subject=state.screen==='menu'&&attract.subject?attract.subject.sections[0].position:state.mode==='free'?target:state.avatar!=='drive'?focus:p;updateWind(subject);placeSun(subject);}
...
async function boot(){try{let loaded=0;streetKit=(await loader.loadAsync(asset('street-kit.glb'))).scene;const lots=await fetch(import.meta.env.BASE_URL+'lots/lots.json').then(r=>r.ok?r.json():{lots:[]}).then(d=>d.lots,()=>[]);environment=buildEnvironment(scene,streetKit,{occupied:occupiedBy(lots)});
 scene.add(await buildLots(lots,{ground:renderedGround,allowed:(x,z,r)=>sceneryClear(x,z,r),allowedLow:(x,z,r)=>sceneryClear(x,z,r,true),modelUrl:src=>import.meta.env.BASE_URL+'lots/models/'+src}));environment.nightMaterials.push(...lotNightMaterials());railway=createRailway(scene,(await loader.loadAsync(asset('tuen-ma-train.glb'))).scene);const model=await loader.loadAsync(asset('art.glb'));vehicle=setupTram(model);manifest=await fetch(asset('asset-manifest.json')).then(r=>{if(!r.ok)throw Error('Asset manifest unavailable');return r.json();});
 ...
 // Five stabled ARTs in their painted bays (src/depot.js); sections 10.6 m apart behind the bay's front end.
 PARKED_BAYS.forEach((bay,n)=>{const parked=setupTram(model,true);parked.isDepot=true;parked.sections.forEach((g,i)=>{const back=6.1+i*10.6;g.position.set(bay.front.x-bay.dir.x*back,DEPOT_Y+.08,bay.front.z-bay.dir.z*back);g.rotation.y=Math.atan2(-bay.dir.x,-bay.dir.z);/* model nose is local −z, as sample().heading */g.name='depot-vehicle-'+(n+1)+'-section-'+i;});depotVehicles.push(parked);});
 makeSignals();makeTraffic();makeStreetLife();makeScooters();makeBirds();makeStopMarkers();makeAvatar();collectSolids(scene,[sky,windParticles,rainEffect,player.group,...[...walkerObjects,...crossers,...plazaPeople,...cyclists,...bikes,...birds,...scooters].map(a=>a.group)]);resetStreetLife();state.s=stopTarget(STOPS[1]);state.lat=5.4;poseTram(vehicle,state.s,state.lat);environment.update(state.s);weather();ready=true;$('#load').classList.add('hidden');show('menu');audio.initAudio();audio.updateMusic(state);requestAnimationFrame(frame);
...
function frame(now){requestAnimationFrame(frame);const rawFrame=(now-last);const dt=clamp(rawFrame/1000,.001,.05);last=now;elapsed+=dt;
 ...
 updateScooters(state.screen==='driving'||state.screen==='menu'?dt:0);
 showDestination(vehicle,travelDirection()===1);for(const a of ai)if(a.active)showDestination(a,a.dir===1);
 const subjectS=state.mode==='turnback'?0:viewS();environment.update(subjectS,elapsed);for(const st of stations)st.root.visible=Math.abs(st.s-subjectS)<600;for(const b of bikes)b.group.visible=Math.abs(b.s-subjectS)<400;aim(dt);railway.update(state.time,camera);audio.updateRailwaySound(railway.trains,camera,state.screen==='driving'||state.screen==='menu',dt);
 {const cam=camera.position,dist=o=>o?cam.distanceTo(o):1e9,pos=a=>a.group?.visible===false?null:a.group?.position||a.sections?.[0].position;
  const running=state.screen==='driving'||state.screen==='menu',driving=state.screen==='driving'&&state.mode!=='free'&&state.avatar==='drive';
  audio.setDrive(driving?Math.abs(state.v)*3.6:0,driving&&(keys.has('KeyW')||keys.has('KeyF')||state.cruise),driving&&!state.park);
  audio.updateWorldSound({people:[...walkerObjects,...crossers,...plazaPeople,...cyclists,...scooters].map(a=>dist(pos(a))),cars:traffic.map(a=>dist(pos(a))),trams:ai.filter(a=>a.active).map(a=>dist(pos(a)))},running);}lightScene();audio.updateMusic(state);for(const a of depotVehicles)for(const g of a.sections)g.visible=Math.abs(subjectS-DEPOT.s)<700;if(hudTick++%5===0)hud();
...
function drawMinimap(){
 ...
 const player=(a,b)=>{x.fillStyle='#ffbf57';x.strokeStyle='#0b0f12';x.lineWidth=1.5;x.beginPath();x.moveTo(a,b-7);x.lineTo(a+6,b+5);x.lineTo(a-6,b+5);x.closePath();x.fill();x.stroke();};
...
function directionMarker(x,a,b,angle,color,size=6){x.save();x.translate(a,b);x.rotate(angle);x.fillStyle=color;x.strokeStyle='#0b0f12';x.lineWidth=1;x.beginPath();x.moveTo(0,-size);x.lineTo(size*.7,size);x.lineTo(0,size*.5);x.lineTo(-size*.7,size);x.closePath();x.fill();x.stroke();x.restore();}
function drawNearbyMap(x){const g=state.avatar!=='drive'?player:state.mode==='free'?sample(state.freeS):vehicle?.sections[0].position||sample(state.s),scale=.14,map=p=>[95+(p.x-g.x)*scale,92+(p.z-g.z)*scale];x.clearRect(0,0,190,190);x.fillStyle='#12171bed';x.fillRect(0,0,190,190);x.strokeStyle='#bbd0c5';x.lineWidth=4;x.beginPath();for(let s=0;s<=LENGTH;s+=10){const p=map(sample(s));s?x.lineTo(...p):x.moveTo(...p);}x.stroke();x.beginPath();for(let i=0;i<=30;i++){const p=map(depotCurve.getPointAt(i/30));i?x.lineTo(...p):x.moveTo(...p);}x.stroke();const dp=map(depotCurve.getPointAt(1));x.fillStyle='#decaf7';x.fillRect(dp[0]-5,dp[1]-5,10,10);x.font='bold 14px sans-serif';x.fillText('Depot',dp[0]+9,dp[1]+5);
 ...
 for(const a of ai)if(a.active){const p=a.sections[0].position,r=sample(a.s);directionMarker(x,...map(p),Math.atan2(r.tx,-r.tz)+(a.dir===1?0:Math.PI),'#51d7ff',5);}const r=sample(state.s);directionMarker(x,...(state.avatar==='drive'?[95,92]:map(vehicle.sections[0].position)),Math.atan2(r.tx,-r.tz)+(state.returning?Math.PI:0),'#ffbf57',8);if(state.avatar!=='drive'){x.fillStyle='#fff';x.strokeStyle='#0b0f12';x.lineWidth=2;x.beginPath();x.arc(95,92,5,0,7);x.fill();x.stroke();}x.fillStyle='#f0f4ec';x.font='12px sans-serif';x.fillText('N ↑',85,20);x.fillText('ROUTE  ⤢',67,177);
}
