import {terminalCurve,depotCurve,l35TrafficCurve} from './routes.js';
...
$('#options-button').onclick=()=>options.showModal();$('#close-options').onclick=()=>options.close();
...
function action(name){if(name==='camera'&&state.mode!=='free'){state.cam=(state.cam+1)%CAMERAS.length;resetOrbit();firstCamera=true;return;}if(name==='sound'){state.muted=!state.muted;audio.setMuted(state.muted);$('#sound').textContent=state.muted?'Sound off':'Sound on';return;}if(name==='pause'){if(state.screen==='driving')show('paused');else if(state.screen==='paused')show('driving');audio.setDrive(0,false);return;}if(state.screen!=='driving'||state.mode==='free')return;
 ...
 case'cab':if(Math.abs(state.v)>.05||state.door>.01){toast('Stop and close doors before changing cab');return;}if(state.s<END_STOP-35){toast('Change cab in the overrun beyond A7');return;}vehicle.sections.reverse();vehicle.flipped=!vehicle.flipped;state.s-=21.2;state.returning=true;state.mode='explore';state.lat=returnOffset(state.s);state.park=true;vehicle.isAI=true;poseTram(vehicle,state.s,state.lat,true);toast('Opposite cab active · depart through the crossover to A1');break;
...
addEventListener('pointerdown',()=>audio.initAudio(),{once:true});addEventListener('keydown',()=>audio.initAudio(),{once:true});
...
function update(dt){state.time+=dt;let onLoop=state.mode==='turnback',p=sample(state.s),v0=state.v,lat0=state.lat;
 ...
 if(state.depotSelected&&state.s>=DEPOT.s-45&&state.s<DEPOT.s){state.depotRoute=true;state.branchDistance=0;state.mode='explore';updateBranch(dt);return;}
...
function hud(){drawMinimap();let st=STOPS[state.index],target=st?stopTarget(st):LENGTH,d=target-state.s,loop=state.mode==='turnback',explore=state.mode==='explore';
 ...
 if(state.returning){$('#next').textContent='Return to A1 / 返回 A1';$('#next-distance').textContent=Math.max(0,state.s-35).toFixed(0)+' m to roundabout';}if(state.depotRoute){$('#next').textContent='Depot / 車廠';$('#next-distance').textContent=Math.max(0,depotCurve.getLength()-state.branchDistance).toFixed(0)+' m to apron';}
...
async function boot(){try{let loaded=0;const model=await loader.loadAsync(asset('art.glb'));vehicle=setupTram(model);manifest=await fetch(asset('asset-manifest.json')).then(r=>{if(!r.ok)throw Error('Asset manifest unavailable');return r.json();});
 for(const st of STOPS){const g=await loader.loadAsync(asset(`station-${st.id}.glb`));const root=g.scene;root.position.copy(point(st.s));root.rotation.set(Math.atan(sample(st.s).grade),sample(st.s).heading,0,'YXZ');root.traverse(n=>{if(n.isMesh){n.castShadow=true;n.receiveShadow=true;if(/_tile$/.test(n.name)){n.material.color.set(0x626866);n.material.roughness=.95;}if(/_platform_.*_light$/.test(n.name))n.visible=false;}});scene.add(root);stations.push({root,s:st.s});$('#load-detail').textContent=`Loading ${st.id} · ${++loaded}/7 stations`;
 }
