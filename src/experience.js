import {terminalCurve,depotCurve,depotExitCurve,l35TrafficCurve} from './routes.js';
import * as T from 'three';
import {GLTFLoader} from 'three/addons/loaders/GLTFLoader.js';
import {RoomEnvironment} from 'three/addons/environments/RoomEnvironment.js';
import {Sky} from 'three/addons/objects/Sky.js';
import {LENGTH,STOPS,sample,fromChainage,toChainage,LOOP,roadSection,project,END_STOP,CROSSINGS,cycleOffset,cycleHeight,DEPOT,laneOffset,curveRadius,returnOffset,CROSSOVER_START,cycleBridgeHeight,CYCLE_BRIDGES} from './alignment.js';
...
<button id="clean-view" aria-label="Hide interface" title="Hide interface" aria-pressed="false">◉</button><div id="top-actions"><label class="operation-control">Headway <select id="headway" aria-label="SGMTS departure headway"><option value="120">2 min</option><option value="180">3 min</option><option value="240">4 min</option><option value="300" selected>5 min</option></select></label><button id="night">Night</button><button id="sound">Sound on</button><button id="camera">C · Camera</button><button id="pause">Ⅱ Pause</button></div>
<section id="menu" class="sheet hidden"><div class="overline">HUNG SHUI KIU / HA TSUEN</div><h1>A new perspective.<br><em>Every journey.</em></h1><p class="subhead">智慧綠色集體運輸系統 · 第一期</p><p class="intro">Drive the articulated electric service through seven stations,<br>or roam freely across the whole map.</p><div class="facts"><span><b>07</b> stations</span><span><b>4.5</b> km profile</span><span><b>04</b> conditions</span></div><div class="choices"><label>Operating conditions<select id="condition"><option value="morning">Morning local · 早晨班次</option><option value="sunset">Golden hour · 黃昏班次</option><option value="rain">Rainy rush hour · 雨天繁忙時段</option><option value="night">Night service · 夜間班次</option></select></label><label>Your drive<select id="mode"><option value="service">Full service · 七站服務</option><option value="access">Accessible service · 無障礙服務</option><option value="priority">Request & priority · 預約停站挑戰</option><option value="turnback">A1 turnback · 總站精準駕駛</option><option value="explore">Explore the whole line · 全線探索</option><option value="free">Free roam · 自由探索</option></select></label></div><button id="menu-music" type="button">♫ Play menu music</button><button id="begin" class="primary">Begin journey <span>開始駕駛 ↗</span></button><p class="note">Drive: W/R/S + A/D · Free roam: WASD + Shift + Space/Ctrl</p><details><summary>Controls & route information</summary><p>Free roam uses WASD, Shift to move faster, Space/Ctrl for altitude, and mouse movement to look. Driving: W forward · R reverse · S brake · G guidance · C camera · click scene to enable mouse-look · Esc releases pointer · F go · T stop · O options · I map · U hide UI · J depot · K cab · L skip · wheel / + / − zoom · 0 reset view · B ramp · Q priority/acknowledge · X door side · Z indicator · H horn · P pause.</p></details></section>
<div id="hud" class="hidden"><div id="next-panel"><small id="service-label">LINE 01 · NORTHBOUND</small><h2 id="next">A1</h2><div id="next-distance"></div><div id="signal-status"></div></div><div id="score-panel"><small>SERVICE SCORE</small><b id="score">0000</b><span id="pax"></span><span id="schedule"></span></div><div id="instruction" role="status"></div><div id="dwell"><i></i><span></span></div><div id="cluster"><div class="speed"><b id="speed">0</b><span>km/h</span><div id="limit">40</div></div><div class="switches"><span id="brake-status"></span><span id="door-status"></span><span id="guide-status"></span><span id="camera-status"></span></div></div><div id="telemetry"></div><div id="route-strip">${STOPS.map(st=>`<div data-stop="${st.id}"><i></i><span>${st.id}</span></div>`).join('')}</div><div id="mobile-buttons"><button data-action="go">Go / 行駛</button><button data-action="stop">Stop / 停車</button><button data-action="doors">Doors / 車門</button><button data-action="camera">Camera / 視角</button></div><div id="route-choices"><button data-action="express">Skip next station</button><button data-action="depot">Depot</button><button data-action="cab">Change cab</button><button data-action="depot-left">Depot exit left · A7</button><button data-action="depot-right">Depot exit right · A1</button></div><canvas id="minimap" width="190" height="430" aria-label="Live route: gold triangle is you, blue rectangles are other ART vehicles, cream circles are stations. Traffic light symbols show each road and its current phase."></canvas><div id="buttons"><button data-action="park">Space · Brake</button><button data-action="doors">E · Doors</button><button data-action="cruise">V · Cruise</button><button data-action="guide">G · Guidance</button><button data-action="ramp">B · Ramp</button><button data-action="priority">Q · Request</button><button data-action="recover">Recover vehicle</button></div></div>
...
for(const id of ['top-actions','route-choices','buttons','telemetry','score-panel'])options.append($('#'+id));
...
$('#options-button').onclick=()=>{document.exitPointerLock?.();keys.clear();options.showModal();};$('#close-options').onclick=()=>options.close();
...
$('#pause').textContent='Ⅱ';$('#pause').setAttribute('aria-label','Pause');$('#camera').textContent='◉';$('#camera').setAttribute('aria-label','Change camera');
...
function contextualPrompt(text){if(text!==lastPrompt){lastPrompt=text;promptUntil=performance.now()+5500;}$('#instruction').textContent=text;$('#instruction').classList.toggle('prompt-visible',performance.now()<promptUntil&&!!text);}
if(mobile)for(const option of [...$('#mode').options])if(['access','priority','free'].includes(option.value))option.remove();
const renderer=new T.WebGLRenderer({antialias:true,powerPreference:'high-performance'});renderer.setSize(innerWidth,innerHeight);renderer.setPixelRatio(Math.min(devicePixelRatio,mobile?1:1.5));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;$('#viewport').appendChild(renderer.domElement);
...
function makeSignals(){
 for(const j of [...junctions,...UNDERPASSES.filter(j=>j.c===3395)]){const r=sample(j.s);if(j.underpass)r.y=r.groundY;else if(r.elevated)continue;const sig={...j,s:j.s-j.stop,centre:j.s,passed:false,heads:[]};signalObjects.push(sig);
  for(const side of [-1,1])for(const axis of ['corridor','side','pedestrian']){const local=new T.Vector3(axis!=='side'?-side*(roadSection(j.s).right+1):side*(roadSection(j.s).right+14),0,axis!=='side'?side*j.stop:side*(j.halfWidth+1)),group=new T.Group();local.applyAxisAngle(new T.Vector3(0,1,0),r.heading);group.position.set(r.x+local.x,r.y,r.z+local.z);group.rotation.y=r.heading+(axis!=='side'?(side===1?0:Math.PI):side*Math.PI/2);
...
function makeTraffic(){
 ...
 for(let i=0;i<4;i++){const source=traffic[i],group=source.group.clone(true);group.name='L35 turning '+source.kind;scene.add(group);traffic.push({...source,group,curve:l35TrafficCurve,routeDistance:i*l35TrafficCurve.getLength()/4,routeDir:i%2?1:-1});}
...
function action(name){if(name==='camera'&&state.mode!=='free'){state.cam=(state.cam+1)%CAMERAS.length;resetOrbit();firstCamera=true;toast(CAMERAS[state.cam]);return;}if(name==='sound'){state.muted=!state.muted;audio.setMuted(state.muted);$('#sound').textContent=state.muted?'Sound off':'Sound on';return;}if(name==='pause'){if(state.screen==='driving')show('paused');else if(state.screen==='paused')show('driving');audio.setDrive(0,false);return;}if(state.screen!=='driving'||state.mode==='free')return;
 switch(name){case'go':if(state.crashed){action('recover');return;}if(tractionLocked({...state,park:false})){toast('Close doors before departure / 請先關門');return;}state.guide=true;state.park=false;state.mobileBrake=false;state.cruise=true;break;
 ...
 case'cab':if(Math.abs(state.v)>.05||state.door>.01){toast('Stop and close doors before changing cab');return;}if(state.depotRoute){if(state.depotOutbound)return;vehicle.sections.reverse();vehicle.flipped=!vehicle.flipped;state.depotOutbound=true;state.branchCurve=depotExitCurve(vehicle.sections[0].position,state.depotExitDir||1);state.branchDistance=0;state.park=true;toast('Opposite cab active · choose exit left to A7 or right to A1 in options');return;}if(state.s<END_STOP-35){toast('Change cab in the overrun beyond A7');return;}vehicle.sections.reverse();vehicle.flipped=!vehicle.flipped;state.s-=21.2;state.returning=true;state.mode='explore';state.lat=returnOffset(state.s);state.park=true;vehicle.isAI=true;poseTram(vehicle,state.s,state.lat,true);toast('Opposite cab active · depart through the crossover to A1');break;
 case'express':{const st=STOPS[state.index];if(!st)return;if(state.s>st.s-90){toast('Choose express before the station fork');return;}state.skipped.push(st.id);state.index++;state.phase='approach';toast(st.id+' bypass selected / 不停站');break;}
 case'stop':state.cruise=false;state.mobileBrake=true;break;
...
const keyActions={Space:'park',KeyE:'doors',KeyV:'cruise',KeyG:'guide',KeyC:'camera',KeyP:'pause',KeyY:'recover',KeyF:'go',KeyT:'stop',KeyK:'cab',KeyJ:'depot',KeyL:'express',BracketLeft:'depot-left',BracketRight:'depot-right',KeyM:'sound',KeyB:'ramp',KeyQ:'priority',KeyX:'side',KeyZ:'indicator'};
addEventListener('keydown',e=>{if(['SELECT','INPUT','TEXTAREA'].includes(e.target.tagName)||e.target.isContentEditable)return;if(e.code==='Escape'){document.exitPointerLock?.();keys.clear();return;}if(e.target.tagName==='BUTTON'&&['Space','Enter'].includes(e.code))return;if(e.repeat)return;const shortcuts={KeyO:'#options-button',KeyI:'#map-button',KeyU:'#clean-view',KeyN:'#night'};if(state.screen==='menu'&&e.code==='KeyM'){$('#menu-music').click();return;}if(state.screen==='driving'&&shortcuts[e.code]){e.preventDefault();$(shortcuts[e.code]).click();return;}if(options.open)return;if(e.code==='Space'||e.code.startsWith('Arrow'))e.preventDefault();keys.add(e.code);if(e.repeat)return;if(e.code==='Enter'){if(state.screen==='menu'||state.screen==='complete')reset();else if(state.screen==='paused')action('pause');}if(keyActions[e.code]&&(state.mode!=='free'||['pause','sound'].includes(keyActions[e.code])))action(keyActions[e.code]);if(e.code==='KeyH'&&state.mode!=='free')audio.playHorn(true);});
...
const buttonKeys={'recover':'Y','go':'F','stop':'T','doors':'E','park':'Space','cruise':'V','guide':'G','ramp':'B','priority':'Q','express':'L','depot':'J','cab':'K','depot-left':'[','depot-right':']'};
for(const b of document.querySelectorAll('[data-action]'))if(buttonKeys[b.dataset.action]){const key=buttonKeys[b.dataset.action];b.title=b.textContent.trim()+' · '+key;b.setAttribute('aria-keyshortcuts',key);}
for(const [id,key] of Object.entries({'camera':'C','pause':'P','options-button':'O','map-button':'I','clean-view':'U','night':'N','menu-music':'M'})){$('#'+id).title=($('#'+id).getAttribute('aria-label')||$('#'+id).textContent.trim())+' · '+key;$('#'+id).setAttribute('aria-keyshortcuts',key);}
...
function update(dt){state.time+=dt;let onLoop=state.mode==='turnback',p=sample(state.s),v0=state.v,lat0=state.lat;
 ...
 if(state.returning||state.depotRoute){updateBranch(dt);return;}
 ...
 if(state.cruise){let desired=(onLoop?12:limit-1)/3.6,decel=state.condition==='rain'?.72:.95;
  if(next&&!mobile&&!['explore','turnback'].includes(state.mode))desired=Math.min(desired,Math.sqrt(2*decel*Math.max(0,target-state.s-.15)));
 ...
 if(onLoop){const base=LOOP.sample(state.loopS),heading=Math.atan2(-base.tx,-base.tz)+state.yaw;let wheel=state.steer*MAX_WHEEL_ANGLE;
  ...
  if(state.guide&&!steer){const ahead=clamp(5+Math.abs(state.v)*.25,5,8),dest=state.s+ahead,desired=laneOffset(dest,1,state.skipped||[]),aim=point(dest,desired),here=point(state.s,state.lat),alpha=angleDelta(Math.atan2(-(aim.x-here.x),-(aim.z-here.z)),heading);wheel=clamp(Math.atan2(2*6.2*Math.sin(alpha),ahead),-.65,.65);}
  ...
  if(state.s>=END_STOP-.1||(state.mode==='explore'&&END_STOP-state.s<5&&Math.abs(state.v)<.15)){state.v=0;state.park=true;if(state.mode==='explore')toast('End of line · Change cab to return to A1');}
 }
...
function actors(dt){const subjectS=viewS();
 ...
 for(const c of traffic){if(c.curve){const length=c.curve.getLength(),u=clamp(c.routeDistance/length,0,1),p=c.curve.getPointAt(u),t=c.curve.getTangentAt(u);let stop=c.routeDistance+c.routeDir*1e6;for(const other of traffic.filter(o=>o!==c&&o.curve===c.curve&&o.routeDir===c.routeDir)){if((other.routeDistance-c.routeDistance)*c.routeDir>0){const d=other.routeDistance-c.routeDir*((other.length+c.length)/2+4);if((d-stop)*c.routeDir<0)stop=d;}}for(const sig of signalObjects.filter(s=>[2650,3395].includes(s.c))){const r=sample(sig.centre),distance=Math.hypot(p.x-r.x,p.z-r.z);if(distance<35&&signalAt(sig,true)!=='go'&&(r.x-p.x)*t.x*c.routeDir+(r.z-p.z)*t.z*c.routeDir>0)stop=c.routeDistance+c.routeDir*Math.max(0,distance-23);}if(state.mode!=='free')for(const body of vehicle.sections){const delta=body.position.clone().sub(p),ahead=(delta.x*t.x+delta.z*t.z)*c.routeDir,cross=Math.abs(delta.x*t.z-delta.z*t.x);if(ahead>0&&cross<4&&Math.abs(delta.y)<4){const block=c.routeDistance+c.routeDir*Math.max(0,ahead-c.length/2-7);if((block-stop)*c.routeDir<0)stop=block;}}const move=approachStep(c.routeDistance,c.v,c.routeDir,7,stop,dt);c.routeDistance=move.position;c.v=move.speed;if(c.routeDistance>length||c.routeDistance<0)c.routeDistance=c.routeDir===1?0:length;const q=c.curve.getPointAt(clamp(c.routeDistance/length,0,1));c.group.position.copy(q);c.group.position.add(new T.Vector3(t.z,0,-t.x).multiplyScalar(c.routeDir*1.6));c.group.position.y+=.08;c.group.rotation.y=Math.atan2(-t.x*c.routeDir,-t.z*c.routeDir);c.group.visible=Math.abs(project(q.x,q.z)-subjectS)<500;if(c.group.visible)contact(c.group,c.width,c.length,c.height,'Turning traffic');continue;}const j=c.junction,r=sample(j.s),sig=signalObjects.find(sig=>sig.c===j.c),go=j.underpass&&!sig||signalAt(sig,true)==='go';
 ...
 for(const p of walkerObjects){const active=state.index===p.station&&state.door>.98&&state.phase==='boarding',served=state.served.includes(STOPS[p.station].id);if(active)p.progress=clamp((state.dwell-p.phase*.2)/(p.alight?2.1:4),0,1);
  ...
  const swing=active&&p.progress>0&&p.progress<1?Math.sin(p.progress*22)*.42:!travel?Math.sin(state.time*3+p.phase)*.12:0;p.limbs.forEach(l=>{l.arm.rotation.x=swing*l.side;l.leg.rotation.x=-swing*l.side;});
 }
...
function aim(dt){const g=vehicle.sections[0],p=g.position,dir=new T.Vector3(0,0,vehicle.flipped?1:-1).applyQuaternion(g.quaternion),left=new T.Vector3(-1,0,0).applyQuaternion(g.quaternion);let target,focus;
 ...
 else if(state.cam===4){target=p.clone().addScaledVector(dir,15);target.y+=.85;focus=p.clone().addScaledVector(dir,-5);focus.y+=1.2;}
...
function hud(){drawMinimap();updateCockpit();let st=STOPS[state.index],target=st?stopTarget(st):LENGTH,d=target-state.s,loop=state.mode==='turnback',explore=state.mode==='explore';
 ...
 if(state.returning){$('#next').textContent='Return to A1 / 返回 A1';$('#next-distance').textContent=Math.max(0,state.s-35).toFixed(0)+' m to roundabout';}if(state.depotRoute){$('#next').textContent='Depot / 車廠';$('#next-distance').textContent=Math.max(0,(state.branchCurve||depotCurve).getLength()-state.branchDistance).toFixed(0)+(state.depotOutbound?' m to main line':' m into depot');}
 ...
 $('#brake-status').textContent=state.park?'● Parking brake applied':'○ Parking brake released';$('#door-status').textContent=state.door>.01?`● ${state.doorSide===1?'Left':'Right'} doors open`:'○ Doors secured';$('#guide-status').textContent=`${state.guide?'●':'○'} Guidance ${state.guide?'on':'off'} · ${state.cruise?'Cruise on':'Manual power'}`;$('#camera-status').textContent=CAMERAS[state.cam]+' · Mouse look · Scroll ± · 0 reset';$('#telemetry').innerHTML=`<span>BATTERY <b>${state.battery.toFixed(1)}%</b></span><span>${state.power<0?'REGEN':'POWER'} <b>${Math.abs(state.power||0).toFixed(0)} kW</b></span><span>GRADE <b>${(sample(state.s).grade*100).toFixed(1)}%</b></span><span>RECOVERED <b>${state.regen.toFixed(2)} kWh</b></span>`;
 ...
 if(loop)instruction=state.loopDone?'Loop complete':state.park?'Space release brake · W drive or V cruise':'Follow the A1 loop and stop at the completion marker';if(state.crashed)instruction='Collision detected · select Recover vehicle / 發生碰撞';contextualPrompt(instruction);$('#dwell').classList.toggle('active',state.phase==='boarding');$('#dwell i').style.width=`${Math.min(100,state.dwell/(state.requiredDwell||1)*100)}%`;$('#dwell span').textContent='PASSENGER EXCHANGE';
...
async function boot(){try{let loaded=0;const model=await loader.loadAsync(asset('art.glb'));vehicle=setupTram(model);manifest=await fetch(asset('asset-manifest.json')).then(r=>{if(!r.ok)throw Error('Asset manifest unavailable');return r.json();});
 ...
 makeSignals();makeTraffic();makeStreetLife();resetStreetLife();state.s=stopTarget(STOPS[1]);state.lat=5.4;poseTram(vehicle,state.s,state.lat);environment.update(state.s);weather();ready=true;$('#load').classList.add('hidden');show('menu');audio.initAudio();audio.updateMusic(state);requestAnimationFrame(frame);
...
function drawMinimap(){
 ...
 x.font='bold 14px sans-serif';for(const st of STOPS){const [a,b]=map(sample(st.s));station(a,b);x.fillStyle='#fff4ce';x.fillText(st.id,a-25,b+5);}
...
function drawNearbyMap(x){const g=state.mode==='free'?sample(state.freeS):vehicle?.sections[0].position||sample(state.s),scale=.14,map=p=>[95+(p.x-g.x)*scale,92+(p.z-g.z)*scale];x.clearRect(0,0,190,190);x.fillStyle='#12372fed';x.fillRect(0,0,190,190);x.strokeStyle='#bbd0c5';x.lineWidth=4;x.beginPath();for(let s=0;s<=LENGTH;s+=10){const p=map(sample(s));s?x.lineTo(...p):x.moveTo(...p);}x.stroke();x.beginPath();for(let i=0;i<=30;i++){const p=map(depotCurve.getPointAt(i/30));i?x.lineTo(...p):x.moveTo(...p);}x.stroke();const dp=map(depotCurve.getPointAt(1));x.fillStyle='#decaf7';x.fillRect(dp[0]-5,dp[1]-5,10,10);x.font='bold 14px sans-serif';x.fillText('Depot',dp[0]+9,dp[1]+5);
 for(const st of STOPS){const [a,b]=map(sample(st.s));x.fillStyle='#fff3cc';x.beginPath();x.arc(a,b,4,0,7);x.fill();x.fillText(st.id,a+9,b+5);}for(const sig of signalObjects.filter(s=>!s.underpass)){const [a,b]=map(sample(sig.centre)),phase=signalAt(sig);x.fillStyle=phase==='go'?'#5cf397':phase==='amber'?'#ffd05a':'#ff7770';x.fillRect(a-5,b-5,10,10);}
 for(const a of ai)if(a.active){const p=a.sections[0].position,r=sample(a.s);directionMarker(x,...map(p),Math.atan2(r.tx,-r.tz)+(a.dir===1?0:Math.PI),'#51d7ff',5);}const r=sample(state.s);directionMarker(x,95,92,Math.atan2(r.tx,-r.tz)+(state.returning?Math.PI:0),'#ffbf57',8);x.fillStyle='#f0f4ec';x.font='12px sans-serif';x.fillText('N ↑',85,20);x.fillText('ROUTE  ⤢',67,177);
}
...
function updateBranch(dt){const braking=state.mobileBrake||keys.has('KeyS'),go=state.cruise||keys.has('KeyW'),locked=tractionLocked(state);state.door=smooth(state.door,state.doorTarget,dt*2.8);state.v=Math.max(0,Math.min(state.depotRoute?4:9,state.v+(locked||braking?-2.5:go?1.1:-.15)*dt));if(braking&&state.v<.1){state.v=0;state.park=true;state.mobileBrake=false;}
 if(state.depotRoute){const curve=state.branchCurve||depotCurve;state.branchDistance=Math.min(curve.getLength(),state.branchDistance+state.v*dt);poseBranch(curve,state.branchDistance);state.s=project(vehicle.sections[0].position.x,vehicle.sections[0].position.z);if(state.branchDistance>=curve.getLength()){if(state.depotOutbound){state.s=DEPOT.s+state.depotExitDir*55;state.lat=laneOffset(state.s,state.depotExitDir);state.returning=state.depotExitDir===-1;vehicle.isAI=state.returning;vehicle.posed=false;state.depotRoute=false;state.yaw=0;}else{state.v=0;state.park=true;}}}else if(vehicle.terminal==='loop'){vehicle.v=state.v;if(!locked&&!braking)terminalStep(vehicle,dt);state.v=vehicle.v;if(!vehicle.terminal){state.returning=false;vehicle.isAI=false;state.s=35;state.lat=laneOffset(35);state.yaw=0;state.index=0;vehicle.posed=false;}}else{let stop=35;for(const sig of signalObjects.filter(s=>!s.underpass)){const line=sig.centre+sig.stop;if(line<state.s&&signalAt(sig)!=='go')stop=Math.max(stop,line);}stop=Math.max(stop,followingStop(state.s,-1,ai));const move=approachStep(state.s,state.v,-1,state.v,stop,dt);state.s=move.position;state.v=move.speed;state.lat=returnOffset(state.s);poseTram(vehicle,state.s,state.lat,true,state.door);for(const d of vehicle.doors){d.node.position.copy(d.base);if(d.side===state.doorSide)d.node.position.z+=state.door*.75*(d.node.name.endsWith('_0')?1:-1);}if(state.s<=35.03){vehicle.terminal='loop';vehicle.loopDistance=0;vehicle.dir=-1;vehicle.s=35;vehicle.active=true;nextDispatch[1]=0;}}
