...
function contextualPrompt(text,persistent=false,key=text){if(key!==lastPrompt){lastPrompt=key;promptUntil=performance.now()+5500;}$('#instruction').textContent=text;$('#instruction').classList.toggle('action',!!persistent);$('#instruction').classList.toggle('prompt-visible',!!text&&(!!persistent||performance.now()<promptUntil));}
...
function holdReason(){const dir=travelDirection();if(signalObjects.some(g=>!g.underpass&&signalAt(g)!=='go'&&(g.centre-state.s)*dir>0&&(g.centre-state.s)*dir<80))return 'the red signal';if(crossers.some(c=>c.active&&(c.s-state.s)*dir>0&&(c.s-state.s)*dir<60))return 'crossing pedestrians';return 'the vehicle ahead';}
...
function hud(){document.body.dataset.depot=String(!!state.depotRoute);drawMinimap();updateCockpit();let st=STOPS[state.index],target=st?stopTarget(st):LENGTH,d=(target-state.s)*travelDirection(),loop=state.mode==='turnback',explore=state.mode==='explore';
 ...
 const [instruction,act,promptKey]=coach(st,d,loop,explore);contextualPrompt(instruction,act,promptKey);$('#dwell').classList.toggle('active',state.phase==='boarding');$('#dwell i').style.width=`${Math.min(100,state.dwell/(state.requiredDwell||1)*100)}%`;$('#dwell span').textContent='PASSENGER EXCHANGE';
...
async function boot(){try{let loaded=0;streetKit=(await loader.loadAsync(asset('street-kit.glb'))).scene;environment=buildEnvironment(scene,streetKit);railway=createRailway(scene,(await loader.loadAsync(asset('tuen-ma-train.glb'))).scene);const model=await loader.loadAsync(asset('art.glb'));vehicle=setupTram(model);manifest=await fetch(asset('asset-manifest.json')).then(r=>{if(!r.ok)throw Error('Asset manifest unavailable');return r.json();});
 ...
 makeSignals();makeTraffic();makeStreetLife();makeBirds();makeStopMarkers();resetStreetLife();state.s=stopTarget(STOPS[1]);state.lat=5.4;poseTram(vehicle,state.s,state.lat);environment.update(state.s);weather();ready=true;$('#load').classList.add('hidden');show('menu');audio.initAudio();audio.updateMusic(state);requestAnimationFrame(frame);
