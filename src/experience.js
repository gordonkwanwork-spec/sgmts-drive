...
for(const id of ['top-actions','buttons','telemetry','score-panel'])options.append($('#'+id));
...
const speedDial=document.createElement('label');speedDial.id='speed-dial';speedDial.innerHTML='<span>SET SPEED</span><output id="target-speed">0</output><small>km/h</small><input id="speed-control" type="range" min="0" max="40" step="1" value="0" aria-label="Target speed in kilometres per hour">';$('#hud').append(speedDial);
...
function action(name){if(name==='camera'&&state.mode!=='free'){state.cam=(state.cam+1)%CAMERAS.length;resetOrbit();firstCamera=true;toast(CAMERAS[state.cam]);return;}if(name==='sound'){state.muted=!state.muted;audio.setMuted(state.muted);$('#sound').textContent=state.muted?'Sound off':'Sound on';return;}if(name==='pause'){if(state.screen==='driving')show('paused');else if(state.screen==='paused')show('driving');audio.setDrive(0,false);return;}if(state.screen!=='driving'||state.mode==='free')return;
 ...
 case'doors':if(Math.abs(state.v)>.05){toast('Stop before opening doors / 請先停車');return;}if(state.ramp>.01){toast('Retract ramp before closing doors / 請收回斜板');return;}state.park=true;state.doorTarget=1-state.doorTarget;if(!state.doorTarget)audio.announce('doors');audio.playDoors(!!state.doorTarget);break;
...
function update(dt){state.time+=dt;let onLoop=state.mode==='turnback',p=sample(state.s),v0=state.v,lat0=state.lat;
 ...
 if(state.cruise){let desired=(mobile?Math.min(state.speedSetting??0,onLoop?12:limit):onLoop?12:limit-1)/3.6,decel=state.condition==='rain'?.72:.95;
  ...
  const pedestrian=crossers.filter(c=>c.active&&c.s-state.s>0).sort((a,b)=>a.s-b.s)[0];if(pedestrian&&!onLoop)desired=Math.min(desired,Math.sqrt(2*decel*Math.max(0,pedestrian.s-state.s-9)));
...
function hud(){document.body.dataset.depot=String(!!state.depotRoute);drawMinimap();updateCockpit();let st=STOPS[state.index],target=st?stopTarget(st):LENGTH,d=(target-state.s)*travelDirection(),loop=state.mode==='turnback',explore=state.mode==='explore';
...
async function boot(){try{let loaded=0;const model=await loader.loadAsync(asset('art.glb'));vehicle=setupTram(model);manifest=await fetch(asset('asset-manifest.json')).then(r=>{if(!r.ok)throw Error('Asset manifest unavailable');return r.json();});
 ...
 makeSignals();makeTraffic();makeStreetLife();makeBirds();resetStreetLife();state.s=stopTarget(STOPS[1]);state.lat=5.4;poseTram(vehicle,state.s,state.lat);environment.update(state.s);weather();ready=true;$('#load').classList.add('hidden');show('menu');audio.initAudio();audio.updateMusic(state);requestAnimationFrame(frame);
