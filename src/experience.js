...
function drawMinimap(){
 const c=$('#minimap'),x=c.getContext('2d'),pts=Array.from({length:240},(_,i)=>sample(LENGTH*i/239));
 ...
 let lastSignalY=0;for(const sig of [...signalObjects].sort((a,b)=>map(sample(a.centre))[1]-map(sample(b.centre))[1])){const [a,roadY]=map(sample(sig.centre)),b=Math.max(roadY,lastSignalY+42),phase=signalAt(sig),color=phase==='go'?'#62f59b':phase==='amber'?'#ffd05a':'#ff7770';
 ...
 for(const a of ai)if(a.active)tram(...map(a.sections[0].position));
 ...
 x.font='14px sans-serif';player(20,349);x.fillStyle='#edf4ef';x.fillText('You',34,354);tram(101,349);x.fillText('Other ART',112,354);
