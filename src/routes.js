...
// ponytail: the corridor is straight beneath VB2, so L35 runs on and meets the D3 kerb square at a left-in/left-out T.
export const l35Mouth={x:mouthX,edge:h3,down:8,up:6};
...
// Out: D6 east arm, left into L35, give way, left into D3's nearside lane (HK keep-left: along -6.5 carries +lateral traffic).
const out=[];for(let x=-100;x<x6-1.8-(h6-1.9)-2;x+=12)out.push(at6(x,-1.9));out.push(...arc(at6,p=>[x6-1.8-(h6-1.9)+(h6-1.9)*Math.sin(p),-h6+(h6-1.9)*Math.cos(p)]));
for(let s=d6.s+h6+3;s<bendS-3;s+=8)out.push(l35Point(s,-l35Offset(s)+1.8));out.push(...bendLane(1.8,0,bendLength-5.5),...arc(at3,p=>[mouthX-10.8+9*Math.cos(p),15.5-9*Math.sin(p)],1));for(let x=mouthX-16.8;x>=-140;x-=12)out.push(at3(x,6.5));
// Return: D3 nearside lane from the far side, left into L35, then left out into the D6 west arm nearside lane.
const back=[];for(let x=140;x>mouthX+11.8;x-=12)back.push(at3(x,6.5));back.push(...arc(at3,p=>[mouthX+9.8-8*Math.sin(p),14.5-8*Math.cos(p)]),...bendLane(-1.8,bendLength-4.5,0,1));
for(let s=bendS-8;s>d6.s+18.5;s-=8)back.push(l35Point(s,-l35Offset(s)-1.8));back.push(...arc(at6,p=>[x6+1.8+10-10*Math.cos(p),-15.5+10*Math.sin(p)]));for(let x=x6+17.8;x<=100;x+=12)back.push(at6(x,-5.5));
...
// Route distances of the front-bumper stop points: D6 stop lines (side phase out, corridor phase back) and the D3 give-way line.
export const L35_LINES={bendS,mergeLat:-(mouthX-10.8),d3:{x:b.x,z:b.z,lx:b.lx,lz:b.lz,tx:b.tx,tz:b.tz},d6Out:find(l35TrafficCurve,p=>local(a,p).x>=-(sideCrossing(d6,-1)+3.5)),giveWay:find(l35TrafficCurve,p=>{const q=local(b,p);return q.z<=h3+1.1&&Math.abs(q.x-mouthX)<4;}),d6Return:find(l35ReturnCurve,p=>{const q=local(a,p);return q.z>=-d6.stop&&q.x>x6;})};
